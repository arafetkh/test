  ...
  Future<void> _updateEmployee() async {
    ...
    try {
      ...
      final updatedEmployee = Employee(
        ...
        username: widget.employeeData['username'], // Keep original username (not editable)
        email: _emailController.text,
        personalEmail: _personalEmailController.text,
        phoneNumber: _phoneController.text,
        firstName: _firstNameController.text,
        lastName: _lastNameController.text,
        password: "", // Password is not updated through this form
        gender: _selectedGender ?? 'MALE',
        maritalStatus: _selectedMaritalStatus ?? 'SINGLE',
        birthDate: _formatDate(_birthDate),
        recruitmentDate: _formatDate(_recruitmentDate),
        role: _selectedRole ?? 'USER',
        type: _selectedType ?? 'OFFICE',
        companyId: _companyIdController.text,
        designation: _designationController.text,
        address: _addressController.text,
        active: widget.employeeData['active'] ?? true, // Maintain active status
      );

      // Call API to update employee
      final result = await EmployeeService.updateEmployee(updatedEmployee);
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: BorderSide(color: AdaptiveColors.primaryGreen),
            ),
          ),
        ),
      ],
