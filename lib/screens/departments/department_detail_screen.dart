...
class DepartmentDetailScreen extends StatefulWidget {
  ...
  final dynamic departmentData; // Contains all department data
  final String departmentKey;   // The string key used in API calls
  final int? departmentId;      // The numeric ID
...
class _AssignEmployeeBottomSheetState extends State<AssignEmployeeBottomSheet> {
  ...
  Future<void> _fetchEmployees() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("${Global.baseUrl}/secure/users-management?size=10000"),
        headers: Global.headers,
      );
        ...
        if (data.containsKey('content') && data['content'] is List) {
          ...
          setState(() {
            _allEmployees = employeesData.map<Map<String, dynamic>>((emp) => {
              'id': emp['id']?.toString() ?? '',  // Keep as string for display
              'rawId': emp['id'],                 // Keep the raw ID value
            ...
            emp['department'] != widget.departmentId
            ).toList();
          ...
          // Log for debugging
          print("Loaded ${_allEmployees.length} employees");
          if (_allEmployees.isNotEmpty) {
            print("First employee: ${_allEmployees[0]}");
          }
        } else {
          setState(() {
            _isLoading = false;
